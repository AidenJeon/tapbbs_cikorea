...
function delete_message(url, timeout_value)
{
    jConfirm(lang['really'], lang['alert'], function (r)
    {
        if (r) {
            $.ajax({
                    type         :'POST', url:BASE_URL + 'user/delete_message', data:$("#delete_message_form").serialize(), timeout:timeout_value * 1000, success:function (data)
                    {
                        var obj = $.parseJSON(data);
...
function delete_friend(idx, url, timeout_value)
{
    jConfirm(lang['really'], lang['alert'], function (r)
    {
        if (r) {
            ...
            $.ajax({
                    type         :'POST', url:BASE_URL + 'user/delete_friend/', data:$("#delete_friend_form").serialize(), timeout:timeout_value * 1000, success:function (data)
                    {
                        var obj = $.parseJSON(data);
...
$(document).ready(function(){
    ...
    $('a[role="message_list"]').click(function(){
        ...
        $.post(BASE_URL + 'user/get_message', {idx : $this.attr('idx'), search : $this.attr('search')}, function(response){
            ...
            $container.find('span[name="name"]').html(response['name']);
            ...
            $reply_link.unbind('click').click(function(){
                ...
                $('#print_receiver_name').html(response['name']);
